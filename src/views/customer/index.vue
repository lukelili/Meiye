  ...
  <div class="container">
    ...
    <div class="my-table">
      ...
      <vTable :table-option="tableOption">
        <template slot="distCount" slot-scope="{ data }">
          {{ data.discount.discount }} 折
        </template>
        <template slot="operation" slot-scope="{ data }" class="operation-btns">
          <a @click="handleJumpDetails(data)">详情</a>
          <a-divider type="vertical" />
          <a @click="handleShowSwiping(data)">划卡</a>
          <a-divider type="vertical" />
          <a>充值</a>
          <a-divider type="vertical" />
          <a @click="handleShowDialog(data)">编辑</a>
        </template>
      </vTable>
    </div>
    <!-- 销卡 -->
